...
<template>
  ...
  >
    ...
    <div class="border-t border-surface-border p-3 space-y-2">
      ...
      <button
        v-for="(label, key) in { gsc: 'GSC Organic', ads: 'Google Ads', profound: 'AI Visibility', transparency: 'Competitor Ads', serper: 'SERP Sweep' }"
        :key="key"
        ...
        @click="runPipeline(key, `/ingest/${key === 'transparency' ? 'transparency' : key === 'serper' ? 'serper-sweep' : key}?days=30`)"
      >
