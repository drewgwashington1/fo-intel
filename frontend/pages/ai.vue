...
function deltaClass(d: { pct: number }) {
  return d.pct > 0 ? 'text-status-up' : d.pct < 0 ? 'text-status-down' : 'text-gray-400'
}
  ...
  return {
    ...
    options: {
      ...
      scales: {
        ...
        y: { min: 0, max: 100, grid: { color: 'rgba(0,0,0,0.06)' }, ticks: { color: '#6B7280', font: { size: 10 } } },
      },
    },
  }
})
  ...
  return {
    ...
    options: {
      ...
      scales: {
        ...
        y: { stacked: true, grid: { color: 'rgba(0,0,0,0.06)' }, ticks: { color: '#6B7280', font: { size: 10 } } },
      },
    },
  }
})
...
const competitorSovChart = computed(() => {
  ...
  return {
    ...
    options: {
      ...
      scales: {
        x: { grid: { color: 'rgba(0,0,0,0.06)' }, ticks: { color: '#6B7280', font: { size: 10 }, callback: (v: any) => `${v}%` } },
  ...
  return {
    ...
    options: {
      ...
      scales: {
        ...
        y: { grid: { color: 'rgba(0,0,0,0.06)' }, ticks: { color: '#6B7280', font: { size: 10 }, callback: (v: any) => `${v}%` }, max: 50 },
      },
    },
  }
})
...
<template>
  <div>
    ...
    <template v-else-if="activeTab === 'visibility' && store.aiOverview">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div
          ...
          class="bg-surface-card rounded-xl p-5 border border-surface-border hover:border-gray-300 transition-colors"
        >
          ...
          <div class="flex items-center gap-2.5 mb-3">
            ...
            <span class="text-xs font-medium text-gray-500 uppercase tracking-wide">#{{ idx + 1 }}</span>
          </div>
          <p class="text-sm font-semibold text-gray-900 mb-3">{{ p.platform }}</p>

          <!-- Large visibility score -->
          <div class="flex items-baseline gap-1 mb-3">
            <span class="text-3xl font-bold text-gray-900">{{ Number(p.avg_visibility).toFixed(1) }}</span>
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border p-5 mb-6">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="text-sm font-semibold text-gray-900">Visibility Score by Platform</h2>
            <p class="text-xs text-gray-400 mt-0.5">Tracking across all {{ store.aiPlatforms?.length ?? 0 }} AI engines over time</p>
          </div>
        </div>
        <div class="h-80">
          <Line v-if="visibilityChart" :data="visibilityChart.data" :options="visibilityChart.options" />
          <div v-else class="h-full flex items-center justify-center text-gray-400 text-sm">No timeline data available</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-surface-card rounded-xl border border-surface-border">
          <div class="px-5 py-4 border-b border-surface-border">
            <h2 class="text-sm font-semibold text-gray-900">Top Topics</h2>
            <p class="text-xs text-gray-400 mt-0.5">Categories where FO appears in AI responses</p>
          </div>
          <div class="divide-y divide-surface-border">
            <div v-for="(t, i) in topics" :key="t.topic" class="px-5 py-3 flex items-center justify-between hover:bg-surface-hover transition-colors">
              <div class="flex items-center gap-3 min-w-0">
                ...
                <span class="text-sm text-gray-900 truncate">{{ t.topic }}</span>
              </div>
              <div class="flex items-center gap-4 shrink-0">
                <div class="text-right">
                  ...
                  <span class="text-[10px] text-gray-400">{{ t.visibility }}/100</span>
                </div>
                <span class="text-xs font-medium text-gray-700 w-12 text-right">{{ t.sov }}%</span>
              </div>
            </div>
            <div v-if="!topics.length" class="px-5 py-8 text-center text-gray-400 text-sm">No topic data available</div>
          </div>
        </div>
        ...
        <div class="bg-surface-card rounded-xl border border-surface-border">
          <div class="px-5 py-4 border-b border-surface-border">
            <h2 class="text-sm font-semibold text-gray-900">Top AI Queries</h2>
            <p class="text-xs text-gray-400 mt-0.5">Actual prompts where FO is mentioned in AI answers</p>
          </div>
            ...
            <div v-for="(p, i) in prompts" :key="p.prompt" class="px-5 py-3 hover:bg-surface-hover transition-colors">
              <div class="flex items-start gap-3">
                ...
                <div class="min-w-0 flex-1">
                  <p class="text-sm text-gray-900 leading-snug">{{ p.prompt }}</p>
                  <div class="flex items-center gap-3 mt-1.5">
                    ...
                    <span class="text-[10px] text-gray-400">SOV {{ p.sov }}%</span>
                  </div>
                </div>
              </div>
            </div>
            <div v-if="!prompts.length" class="px-5 py-8 text-center text-gray-400 text-sm">No prompt data available</div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="activeTab === 'citations' && store.aiOverview">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-surface-card rounded-xl p-5 border border-surface-border">
          ...
          <div class="flex items-baseline gap-2">
            ...
            <p class="text-sm font-medium" :class="citationTrend.direction === 'up' ? 'text-status-up' : citationTrend.direction === 'down' ? 'text-status-down' : 'text-gray-400'">
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border p-5 mb-6">
        <div class="mb-4">
          <h2 class="text-sm font-semibold text-gray-900">Citation Trend</h2>
          <p class="text-xs text-gray-400 mt-0.5">Direct, indirect, and mention citations over time</p>
        </div>
        <div class="h-72">
          <Bar v-if="citationChart" :data="citationChart.data" :options="citationChart.options" />
          <div v-else class="h-full flex items-center justify-center text-gray-400 text-sm">No citation timeline data available</div>
        </div>
      </div>
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border mb-6">
        <div class="px-5 py-4 border-b border-surface-border">
          <h2 class="text-sm font-semibold text-gray-900">Most Cited URLs Across AI Engines</h2>
          <p class="text-xs text-gray-400 mt-0.5">Pages most frequently referenced in AI-generated answers</p>
        </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-for="(c, idx) in citedUrls" :key="c.url" class="border-b border-surface-border last:border-0 hover:bg-surface-hover transition-colors">
                ...
                <td class="px-5 py-3 font-medium truncate max-w-[400px]" :class="c.url.includes('firstorion') ? 'text-fo-action' : 'text-gray-700'" :title="c.url">{{ c.url }}</td>
                <td class="px-5 py-3 text-right text-gray-900 font-bold">{{ fmtNum(c.citations) }}</td>
                <td class="px-5 py-3">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-2 bg-surface-card rounded-xl border border-surface-border">
          <div class="px-5 py-4 border-b border-surface-border">
            <h2 class="text-sm font-semibold text-gray-900">First Orion Cited Pages</h2>
            <p class="text-xs text-gray-400 mt-0.5">FO pages referenced in AI engine responses</p>
          </div>
          <div class="divide-y divide-surface-border">
            <div v-for="(c, idx) in citedUrls.filter((u: any) => u.url.includes('firstorion'))" :key="c.url" class="px-5 py-3 flex items-center justify-between hover:bg-surface-hover transition-colors">
              ...
              <span class="text-sm font-bold text-gray-900 shrink-0 ml-4">{{ fmtNum(c.citations) }}</span>
            </div>
            <div v-if="!citedUrls.filter((u: any) => u.url.includes('firstorion')).length" class="px-5 py-8 text-center text-gray-400 text-sm">No FO citations found</div>
          </div>
        </div>
        ...
        <div class="bg-surface-card rounded-xl border border-surface-border">
          <div class="px-5 py-4 border-b border-surface-border">
            <h2 class="text-sm font-semibold text-gray-900">Citations by Engine</h2>
          </div>
          <div class="p-5 space-y-4">
            <div v-for="p in citationsByPlatform" :key="p.platform" class="flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-1">
                  <span class="text-xs font-medium text-gray-900">{{ p.platform }}</span>
                  <span class="text-xs text-gray-700 font-medium">{{ fmtNum(p.citations) }}</span>
                </div>
            ...
            <div v-if="!citationsByPlatform.length" class="text-center text-gray-400 text-sm py-6">
    ...
    <template v-else-if="activeTab === 'competitors' && store.aiOverview">
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border p-5 mb-6">
        <div class="mb-4">
          <h2 class="text-sm font-semibold text-gray-900">FO vs Competitors — Share of Voice</h2>
          <p class="text-xs text-gray-400 mt-0.5">Average SOV across all AI engine categories</p>
        </div>
        <div class="h-72">
          <Bar v-if="competitorSovChart" :data="competitorSovChart.data" :options="competitorSovChart.options" />
          <div v-else class="h-full flex items-center justify-center text-gray-400 text-sm">No competitor data available</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-surface-card rounded-xl border border-surface-border">
          <div class="px-5 py-4 border-b border-surface-border">
            <h2 class="text-sm font-semibold text-gray-900">Competitor Rankings</h2>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
              ...
              <tbody>
                <tr v-for="(c, idx) in sortedCompetitors" :key="c.competitor_domain" class="border-b border-surface-border last:border-0 hover:bg-surface-hover transition-colors">
                  <td class="px-5 py-3 text-gray-400 text-xs font-medium">{{ idx + 1 }}</td>
                  <td class="px-5 py-3">
                    <div class="flex items-center gap-2">
                      <div class="w-2 h-2 rounded-full shrink-0" :style="{ backgroundColor: competitorColors[c.competitor_domain] || '#8B95A5' }" />
                      <span class="text-gray-900 font-bold text-sm">{{ c.competitor_domain }}</span>
                    </div>
                  </td>
                  <td class="px-5 py-3">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-gray-700 text-xs font-medium">{{ fmtPct(c.avg_sov) }}</span>
                    </div>
                  </td>
                  <td class="px-5 py-3 text-right text-gray-700">{{ fmtNum(c.citations) }}</td>
                  ...
                  <td class="px-5 py-3 text-right text-gray-400 text-xs">—</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="bg-surface-card rounded-xl border border-surface-border">
          <div class="px-5 py-4 border-b border-surface-border">
            <h2 class="text-sm font-semibold text-gray-900">SOV by Category</h2>
            <p class="text-xs text-gray-400 mt-0.5">First Orion vs competitors across categories</p>
          </div>
          <div class="p-5">
            <div class="h-80">
              <Bar v-if="sovCategoryChart" :data="sovCategoryChart.data" :options="sovCategoryChart.options" />
              <div v-else class="h-full flex items-center justify-center text-gray-400 text-sm">No category data available</div>
            </div>
          </div>
        </div>
      </div>
