  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-0">
      <div class="flex items-center gap-3">
        <h1 class="text-xl font-semibold text-white">{{ tabTitle }}</h1>
        <button class="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-300 transition-colors">
    ...
    <div class="flex items-center gap-0 py-3 mt-0 mb-4">
      <div class="flex items-center gap-0 bg-surface-card rounded-lg border border-surface-border overflow-hidden">
        <button
          ...
          class="flex items-center gap-1.5 px-4 py-2 text-xs font-medium text-gray-400 hover:text-white transition-colors"
          :class="{ 'border-r border-surface-border': i < filterButtons.length - 1 }"
        >
          {{ f.label }}
          <svg v-if="f.icon" class="w-3 h-3 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path d="M19 9l-7 7-7-7" />
          </svg>
        </button>
      </div>
      <button class="flex items-center gap-1.5 px-4 py-2 ml-2 text-xs font-medium text-gray-500 hover:text-gray-300 transition-colors">
        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <template v-else-if="store.organicOverview && activeTab === 'keywords'">
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border p-5 mb-6">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-sm font-semibold text-white">Clicks & Impressions Trend</h2>
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border mb-6">
        <div class="px-5 py-4 border-b border-surface-border flex items-center justify-between">
          <h2 class="text-sm font-semibold text-white">Keywords</h2>
          <span class="text-xs text-gray-500">{{ store.organicTopQueries.length }} keywords</span>
        </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-5 py-3 text-right text-gray-300 font-medium">{{ fmtNum(q.clicks) }}</td>
    ...
    <template v-else-if="store.organicOverview && activeTab === 'pages'">
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border p-5 mb-6">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="text-sm font-semibold text-white">Organic traffic by page</h2>
      ...
      <div class="flex items-center gap-4 mb-4 px-1">
        <span class="text-sm text-gray-400">
          <span class="text-white font-semibold">{{ totalPages }}</span> pages
        </span>
        <span class="text-surface-border">|</span>
        <span class="text-sm text-gray-400">
          Total traffic: <span class="text-white font-semibold">{{ fmtNum(totalPageTraffic) }}</span>
        </span>
        <span class="text-surface-border">|</span>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-5 py-3 text-left text-gray-500 text-xs">—</td>
                ...
                <td class="px-5 py-3 text-right text-xs" :class="p.prev_clicks != null && p.clicks > p.prev_clicks ? 'text-status-up' : p.clicks < (p.prev_clicks || 0) ? 'text-status-down' : 'text-gray-500'">
    ...
    <template v-else-if="store.organicOverview && activeTab === 'competitors'">
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border p-5 mb-6">
        <h2 class="text-sm font-semibold text-white mb-2">Competing Domains</h2>
        <p class="text-xs text-gray-500 mb-4">Keyword overlap and traffic share visualization</p>
        <div class="h-80 rounded-lg border border-dashed border-surface-border bg-surface/50 flex flex-col items-center justify-center gap-3">
          ...
          <div class="relative w-full h-full">
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <span class="text-xs text-gray-500 bg-surface-card/80 px-3 py-1.5 rounded-lg border border-surface-border">
      ...
      <div class="bg-surface-card rounded-xl border border-surface-border mb-6">
        <div class="px-5 py-4 border-b border-surface-border">
          <h2 class="text-sm font-semibold text-white">Top competing domains</h2>
          <p class="text-xs text-gray-500 mt-0.5">Domains competing for the same organic keywords</p>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              ...
              >
                ...
                <td class="px-5 py-3">
                  <div class="flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full" :class="comp.dotColor" />
                    <span class="text-white font-medium text-xs">{{ comp.domain }}</span>
                  </div>
                </td>
                <td class="px-5 py-3 text-right text-gray-500 text-xs">&mdash;</td>
                <td class="px-5 py-3 text-right text-gray-500 text-xs">&mdash;</td>
                <td class="px-5 py-3 text-right text-gray-500 text-xs">&mdash;</td>
                <td class="px-5 py-3 text-right text-gray-500 text-xs">&mdash;</td>
              </tr>
